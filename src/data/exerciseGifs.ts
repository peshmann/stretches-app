// Exercise GIF demonstrations — curated from multiple free sources
// User-verified via pick-gifs.html on 2026-04-12
// Sources: ExerciseDB (MIT), MakeAGif (user-generated), Tenor (public)

export const exerciseGifs: Record<string, string> = {
  // ═══ Verified correct (from review-gifs.html) ═══

  'dead-bug': 'https://static.exercisedb.dev/media/iny3m5y.gif',
  'plank': 'https://static.exercisedb.dev/media/VBAWRPG.gif',
  'side-plank': 'https://static.exercisedb.dev/media/5VXmnV5.gif',
  'pallof-press': 'https://static.exercisedb.dev/media/9pa4H5m.gif',
  'cable-rows': 'https://static.exercisedb.dev/media/hvV79Si.gif',
  'lat-pulldown': 'https://static.exercisedb.dev/media/LEprlgG.gif',
  'glute-bridge': 'https://static.exercisedb.dev/media/qKBpF7I.gif',
  'glute-bridge-activation': 'https://static.exercisedb.dev/media/qKBpF7I.gif',
  'goblet-squat': 'https://static.exercisedb.dev/media/yn8yg1r.gif',
  'reverse-lunges': 'https://static.exercisedb.dev/media/SSsBDwB.gif',
  'hamstring-stretch': 'https://static.exercisedb.dev/media/99rWm7w.gif',
  'wall-angels': 'https://i.makeagif.com/media/10-16-2019/6Oc6Qm.gif',
  'bird-dog': 'https://i.makeagif.com/media/11-19-2014/E7Spmi.gif',
  'thread-the-needle': 'https://i.makeagif.com/media/2-17-2023/9AdAkA.gif',
  'thread-the-needle-priority': 'https://i.makeagif.com/media/2-17-2023/9AdAkA.gif',
  'single-leg-glute-bridge': 'https://media.tenor.com/6e9R6ktpmjQAAAAC/single-leg-butt-bridge.gif',
  'clamshells': 'https://media.tenor.com/6I8f5HYyIq0AAAAC/clam-exercise.gif',
  'cat-cow': 'https://media.tenor.com/LPwFzdwK9AEAAAAC/cat-cow-stretch.gif',
  'cat-cow-stretch': 'https://media.tenor.com/LPwFzdwK9AEAAAAC/cat-cow-stretch.gif',
  'foam-roll-quads': 'https://media.tenor.com/L5Cf_i73nXcAAAAC/rouleau.gif',
  'lat-stretch': 'https://static.exercisedb.dev/media/f38OEuO.gif',

  // ═══ User-picked from candidates (pick-gifs.html) ═══

  'neck-stretches': 'https://static.exercisedb.dev/media/x2chWLO.gif',
  'figure-4-stretch': 'https://static.exercisedb.dev/media/RQNVT10.gif',
  'light-cardio-a': 'https://static.exercisedb.dev/media/H1PESYI.gif',
  'light-cardio-b': 'https://static.exercisedb.dev/media/H1PESYI.gif',
  'cable-face-pulls': 'https://i.makeagif.com/media/2-19-2024/i4nFKk.gif',
  'childs-pose': 'https://i.makeagif.com/media/5-29-2014/GLydU_.gif',
  'childs-pose-priority': 'https://i.makeagif.com/media/5-29-2014/GLydU_.gif',

  // ═══ User-picked round 2 ═══

  'doorway-chest-stretch': 'https://i.makeagif.com/media/5-04-2025/IuVHwU.gif',
  'doorway-chest-priority': 'https://i.makeagif.com/media/5-04-2025/IuVHwU.gif',
  'foam-roll-hip-flexors': 'https://i.makeagif.com/media/8-20-2015/H3b1Kr.gif',
  'foam-roll-lats': 'https://static.exercisedb.dev/media/c3Pfhti.gif',

  // ═══ User-picked final round ═══

  'kneeling-hip-flexor': 'https://i.makeagif.com/media/6-03-2014/xjfID0.gif',
  'hip-flexor-priority': 'https://i.makeagif.com/media/6-03-2014/xjfID0.gif',
  'band-pull-aparts': 'https://i.makeagif.com/media/2-26-2018/mAf82b.gif',

  // ═══ No match found (4 exercises) — text + YouTube link only ═══
  // seated-spinal-twist, deep-neck-flexor, thoracic-extension, foam-roll-thoracic
};
